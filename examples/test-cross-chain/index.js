...
const TestCrossChain = require('../../build/TestCrossChain.json');
const Gateway = require('../../build/IAxelarGateway.json');
const IERC20 = require('../../build/IERC20.json');

async function deploy(chain, wallet) {
    console.log(`Deploying TestCrossChain for ${chain.name}.`);
    const contract = await deployContract(wallet, TestCrossChain, [chain.gateway, chain.gasReceiver]);
    chain.testCrossChain = contract.address;
    console.log(`Deployed DistributionExecutable for ${chain.name} at ${chain.testCrossChain}.`);
}
...
async function test(chains, wallet, options) {
    ...
    for(const chain of [source, destination]) {
        ...
        chain.contract = new Contract(chain.testCrossChain, TestCrossChain.abi, chain.wallet);
    ...
    await (await source.contract.sendToFirst(
        destination.name,
        destination.testCrossChain,
        accounts, 
        'aUSDC',
        amount,
